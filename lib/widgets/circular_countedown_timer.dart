...
import 'package:googly/src/custom_timer_painter.dart';
...
class CircularCountDownTimerState extends State<CircularCountDownTimer>
  ...
  void _setController() {
    ...
    if (widget.initialDuration > 0 && widget.autoStart) {
      if (widget.isReverse) {
        controller?.value = 1 - (widget.initialDuration / widget.duration);
      } else {
        controller?.value = (widget.initialDuration / widget.duration);
      }
                    ...
                    Positioned.fill(
                      child: CustomPaint(
                        painter: CustomTimerPainter(
                            animation: _countDownAnimation ?? controller,
...
class CountDownController {
  ...
  CircularCountDownTimerState get timeState=>_state;
  ...
  void pause() {
    _state.controller?.stop(canceled: false);
  }

  /// This Method Resumes the Countdown Timer
  void resume([double? value]) {
    if (_isReverse) {
      _state.controller?.reverse(from: (_state.controller!.value+value!).clamp(0.0, 1.0));
    } else {
      _state.controller?.forward(from: _state.controller!.value);
    }
  }
  ...
  void restart({int? duration}) {
    _state.controller!.duration =
        Duration(seconds: duration ?? _state.controller!.duration!.inSeconds);
    if (_isReverse) {
      _state.controller?.reverse(from: 1);
    } else {
      _state.controller?.forward(from: 0);
    }
  }
  ...
  String getTime() {
    return _state
        ._getTime(_state.controller!.duration! * _state.controller!.value);
  }
}
